...
path("users", () -> {
    ...
    path("{userId}", () -> {
        ...
        patch(UserController::upappeUser, Role.USER_WRITE);
...
attribut, så vi kan gemme brugerens rolle i appabasen. Man kan forestille sig et system hvor hver bruger kan have flere 
...
### Password hashing
Husk at bruge bcrypt til at hashe kodeordet før det gemmes i appabasen.
...
## SecurityDAO
Vi har brug for at kunne oprette og hente brugere fra appabasen, at kunne tilføje og fjerne roller fra brugere. Vi kan 
...
public class SecurityDao implements ISecurityDao
{
    ...
    public UserAccount addRole(UserAccount user, Role role)
    {
        try (EntityManager em = emf.createEntityManager())
        {
            ...
            UserAccount upappedUser = em.merge(foundUser);
            em.getTransaction().commit();
            return upappedUser;
        }
    }
